...
export function fixAssetSize(asset: IAsset): boolean {
  ...
  let fixedWidth = closestNumber(canvas.width, metadata.baseSize.w);

  const fixedHeight = closestNumber(canvas.height, metadata.baseSize.h);

  // Prevention
  fixedWidth = ratio * fixedHeight;

  if (fixedWidth === canvas.width && fixedHeight === canvas.height) {
