...
function resizeBoardCanvas(board: Board<Canvas>) {
  board.call((cell) => {
    ...
    if (w > maxW || h > maxH) {
      cell.content = resizeCanvas(cell.content, { w: maxW, h: maxH });
    }
  });
}
...
function processBoardCanvas(
  ...
  board.call((cell) => {
    let position = board.getPosition({ x: cell.x, y: cell.y });

    position.x += (board.config.cellWidth - cell.content.width) / 2;
    position.y += (board.config.cellHeight - cell.content.height) / 2;

    ctx.drawImage(cell.content, position.x, position.y);
  });

  return new MinimalAsset(DEFAULT_METADATA).loadFromCanvas(canvas);
}
...
export function createSkinOverview(
  ...
  const skinWeapon = new SkinFull().setSkin(skin).setGameskin(gameskin);

  for (let i = 0; i < amount; i++) {
    skin.setEyeAssetPart(EMOTES.at(i));

    skinWeapon.setWeapon(WEAPONS.at(i)).process();
